...
## Cross-version CI (all-versions matrix is green as of 2026-05-20)
The fork's CI builds a **Mac × Windows × AC25–29 matrix** (inherited from upstream). I only had the AC29 DevKit locally, so cross-version issues only showed in CI — fixed iteratively:
- **`ACAPI_View_Get/Change3DProjectionSets`** (Set3DProjection), **`ACAPI_Navigator_NewNavigatorView`** (CreateView), **`ACAPI_Navigator_Get/ChangeViewOptions`** (SetModelViewOptions) don't exist in older API DevKits → guarded those three Execute bodies at `#if defined(ServerMainVers_2900)` with a "requires Archicad 29" stub on older versions. **So those 3 commands are AC29-only**; the threshold could be lowered if confirmed on an older DevKit.
- MSVC treats unused-parameter as error (`/WX` → C2220); the stub branches don't use `parameters`, so added `(void) parameters;` (clang ignored it via `-Wno-unused-parameter`, so Mac passed first).
