...
| [#394](https://github.com/ENZYME-APD/tapir-archicad-automation/pull/394) | `OpenView` | 🟡 OPEN, ready for review | Branch `Yostage:feat/open-view`, commit `b4920b3`. AC27+ guarded (`ACAPI_View_GoToView` not on AC25/26). No reviewer comments yet. |
...
| — | `CreateView` | 🔧 WIP, lowest priority | Returns `0x8106006a` from `NewNavigatorView`; OpenView covers nav. Not in upstream queue. |
...
- **Fork-side CI iteration pattern**: push to `ci/<name>-check` throwaway branch first to run the full matrix on the fork (workflow triggers on push). Iterate without involving the maintainer. Only after green, push to `feat/<name>` and open the upstream PR. Established by CreateTexts; reused by OpenView. Delete the throwaway branch once the real one is up.
...
- **Version string in `RegisterCommand<>`**: looks like add-on version when the command was authored, not strictly monotonic. Match the latest semantically (CreateTexts and OpenView both used `"1.4.2"`).
