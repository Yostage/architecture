...
| [#398](https://github.com/ENZYME-APD/tapir-archicad-automation/pull/398) | `ChangeWindow` — add optional `navigatorItemId` | 🟡 OPEN, ready for review | Branch `Yostage:feat/change-window-to-view`, squashed to single commit `4c57482`. Schema-additive, non-breaking. AC27+ routes through `ACAPI_View_GoToView`; AC25/26 rejects with `NOTSUPPORTED` (silent fallback dropped — see lesson below). Fork CI matrix green; visually verified in AC29 that activating saved views applies their saved layer combinations (Floor Plans vs Structural Plans vs Ceiling Plans on the same story produced distinct displays). |
